...
vel limits, (deg/sec): 200,200,260,360,360,450

range of motion (deg): 360,200,280, 400, 230, 800
...
note: 140 has offset shoulder from base (a1 != 0), whereas 120 DOES  have offset
120 has offset from elbow to forearm, appears that 140 does not
...
const int NJNTS=6;
const int MAX_SOLNS_RETURNED=1; //SPECIAL CASE: just return ONE solution
                                // specialize on "forward", "elbow up" and positive wrist bend

// a fwd kin solver...
// list DH params here
// these values from Matlab "ARTE" agree with URDF from SWRI
//robot.DH.a='[0 0.270 0.070 0 0 0]';
const double DH_a1=0.070;
const double DH_a2=0.360;
...
const double DH_d1 = 0.352; //0.290;
const double DH_d2 = 0.0;
const double DH_d3 = 0.0;
const double DH_d4 = 0.380;
const double DH_d5 = 0.0;
const double DH_d6 = 0.065;
...
const double DH_q_max1 = deg2rad*175;
const double DH_q_max2 = deg2rad*90;
const double DH_q_max3 = deg2rad*70;
const double DH_q_max4 = deg2rad*178;
const double DH_q_max5 = deg2rad*115;
const double DH_q_max6 = deg2rad*179; //deg2rad*400; //deliberately reduce this, to avoid excess solutions

const double DH_q_min1 = -deg2rad*175;
const double DH_q_min2 = -deg2rad*110;
const double DH_q_min3 = -deg2rad*230; //
const double DH_q_min4 = -deg2rad*178;
const double DH_q_min5 = -deg2rad*115;
const double DH_q_min6 = -deg2rad*179; //-deg2rad*400;
...
class Irb140_fwd_solver {
public:
    Irb140_fwd_solver(); //constructor; //const hand_s& hs, const atlas_frame& base_frame, double rot_ang);
    ...
    void  test_q123(std::vector<Vectorq6x1> q6dof_solns);
private:
...
class Irb140_IK_solver: public Irb140_fwd_solver {
public:
    Irb140_IK_solver(); //constructor; 
...
//the following merely renames the irb140 solvers to a generic name
class FwdSolver {
public:
