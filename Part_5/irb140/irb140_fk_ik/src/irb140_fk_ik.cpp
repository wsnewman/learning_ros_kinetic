// irb140_kinematics implementation file; start w/ fwd kin

#include <irb140_fk_ik/irb140_kinematics.h>
using namespace std;
...
Irb140_fwd_solver::Irb140_fwd_solver() { 

    ROS_INFO("fwd_solver constructor");
}

Eigen::MatrixXd Irb140_fwd_solver::jacobian(const Eigen::VectorXd& q_vec) {
  Eigen::MatrixXd jacobian;
// ... do some work here FINISH ME!
  //jacobian = irb140_fwd_solver.jacobian(q_vec);
  return jacobian;
}
...
Eigen::Affine3d Irb140_fwd_solver::fwd_kin_solve(const Vectorq6x1& q_vec) {
    Eigen::Matrix4d M;
...
Eigen::Matrix4d Irb140_fwd_solver::get_wrist_frame() {
    return A_mat_products[4];
}

//alternative fnc: accepts arg of type Eigen::VectorXd 
//USES ABB COORDS
Eigen::Affine3d Irb140_fwd_solver::fwd_kin_solve(const Eigen::VectorXd& q_vec) {
...
void  Irb140_fwd_solver::test_q123(std::vector<Vectorq6x1> q6dof_solns) {
...
Eigen::Matrix4d Irb140_fwd_solver::fwd_kin_solve_(const Vectorq6x1& q_vec) {
...
Irb140_IK_solver::Irb140_IK_solver() {
    //constructor: 
    L_humerus = DH_a_params[1];
    double L3 = DH_d_params[3];
    double A2 = DH_a_params[2];
    L_forearm = L3; //sqrt(A2 * A2 + L3 * L3);
    
    //for A2 = 0, do  not need this angle correction; set it to zer0
    phi_elbow=0.0; //acos((A2*A2+L_forearm*L_forearm-L3*L3)/(2.0*A2*L_forearm));
}

int Irb140_IK_solver::ik_solve(Eigen::Affine3d const& desired_hand_pose,  std::vector<Eigen::VectorXd> &q_ik_solns) {
...
int Irb140_IK_solver::ik_solve(Eigen::Affine3d const& desired_hand_pose) {
    q6dof_solns.clear();
    bool reachable = compute_q123_solns(desired_hand_pose, q6dof_solns);
    if (!reachable) {
        return 0;
    }
    //test_q123(q6dof_solns);
    reachable = false;
...
void Irb140_IK_solver::get_solns(std::vector<Vectorq6x1> &q_solns) {
    q_solns = q_solns_fit; //q6dof_solns;
}

//given desired flange pose, fill up solns for q1, q2, q3 based on wrist position

bool Irb140_IK_solver::compute_q123_solns(Eigen::Affine3d const& desired_hand_pose, std::vector<Vectorq6x1> &q_solns) {
...
bool Irb140_IK_solver::solve_for_theta2(Eigen::Vector3d w_wrt_1, double r_goal, double q2_solns[2]) {
...
bool Irb140_IK_solver::solve_for_theta3(Eigen::Vector3d w_wrt_1, double r_goal, double q3_solns[2]) {
     //phi_elbow;
    ...
    double testx = L_humerus - L_forearm*sin(q3_solns[0]);
    double testy = L_forearm*cos(q3_solns[0]);
    double testr = sqrt(testx*testx+testy*testy);
    cout<<"rgoal = "<<r_goal<<endl;
    cout<<"q3[0] = "<<q3_solns[0]<<" --> r = "<<testr<<endl;
     testx = L_humerus +- L_forearm*sin(q3_solns[1]);
     testy = L_forearm*cos(q3_solns[1]);
...
bool Irb140_IK_solver::fit_q_to_range(double q_min, double q_max, double &q) {
    while (q<q_min) {
        q+= 2.0*M_PI;
    }
    while (q>q_max) {
...
bool Irb140_IK_solver::fit_joints_to_range(Vectorq6x1 &qvec) {
    bool fits=true;
    bool does_fit;
    double q;
    for (int i=0;i<6;i++) {
...
bool Irb140_IK_solver::solve_spherical_wrist(Vectorq6x1 q_in,Eigen::Matrix3d R_des, std::vector<Vectorq6x1> &q_solns) {
...
Eigen::Affine3d FwdSolver::fwd_kin_solve(Eigen::VectorXd const& q_vec) { // given vector of q angles, compute fwd kin
 ...
 fwd_soln = irb140_fwd_solver.fwd_kin_solve(q_vec);
 return fwd_soln;
}
...
int IKSolver::ik_solve(Eigen::Affine3d const& desired_hand_pose,  std::vector<Eigen::VectorXd> &q_ik_solns) {
  ...
  int nsolns = irb140_IK_solver.ik_solve(desired_hand_pose,  q_ik_solns);
  return nsolns;
}
 * */
