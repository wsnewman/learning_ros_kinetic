// irb140_kinematics_test_main.cpp
// wsn, Nov 2018
// test function for irb140_kinematics library
...
int main(int argc, char **argv) {
    ros::init(argc, argv, "irb140_kinematics_test_main");
    ros::NodeHandle nh;
    ...
    Irb140_IK_solver ik_solver;
    ros::Subscriber joint_state_sub = nh.subscribe("irb140/joint_states", 1, jointStatesCb);
    std::cout << "==== Test for irb140 kinematics solver ====" << std::endl;
    ...
    while (ros::ok()) {
        ...
        Eigen::Affine3d A_fwd_DH = irb140_fwd_solver.fwd_kin_solve(q_in); //fwd_kin_solve
        ...
        A_wrist = irb140_fwd_solver.get_wrist_frame();
        ...
        for (int i = 0; i < nsolns; i++) {
            A_fwd_DH = irb140_fwd_solver.fwd_kin_solve(q6dof_solns[i]);
            A_wrist = irb140_fwd_solver.get_wrist_frame();
