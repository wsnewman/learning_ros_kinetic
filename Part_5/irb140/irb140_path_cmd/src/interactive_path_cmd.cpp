...
{
    // ROS set-ups:
    ros::init(argc, argv, "interactive_path_move"); //node name
    ...
    int ans;
    double des_ang;
    cout<<"enter jnt num, 0 through 5 (negative to quit): ";
    cin>>ans;
    while ((ans>=0)&&(ans<6)) {
       ...
       for (int i=0;i<VECTOR_DIM;i++) {
          q_vec_arm_pose[i] = g_q_vec_arm_Xd[i];
       }
    cout<<"enter desired joint angle value: ";
    cin>>des_ang;
    q_vec_arm_pose[ans]=des_ang;
